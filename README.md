...
- **模拟真题**：有一些题型只是背概念可能还不够，这类题含有一定情景、需要动手画一画，但是又缺乏真题，比方说CRC校验；还有一些题有一套系统的知识点，但是只考了一个点，需要举一反三。我对于这类题自己增加了**模拟真题**。
...
>
> **选项分析**：
> 这是一个标准术语题。**CSMA/CD** 是有线以太网的核心协议：**Carrier Sense** (先听后发)、**Multiple Access** (多台电脑共用总线)、**Collision Detection** (边发边听，冲突停发)。
>
...
> ![image-20251230103953766](./assets/image-20251230103953766.png)
>
> **24. 选择题**：
>
> **虽然初始连接协议对于那些可以按需创建的服务器运行良好，但在许多情况下，服务是独立于进程服务器而存在的。例如，文件服务器需要在特殊硬件（有磁盘的机器）上运行，不能在有人想与之通信时临时创建。为了处理这种情况，通常使用另一种方案。在这种模型中，存在一个特殊的进程，称为名称服务器 (Name Server)，有时也称为 ______。 (单选题 2分)**
>
> - A. 进程服务器 (Process server)
> - B. 文件服务器 (File server)
> - C. 代理服务器 (Proxy server)
> - D. 目录服务器 (Directory server)
>
> **选项分析**：
> 在分布式系统或网络服务模型中，**名称服务器 (Name Server)** 的主要功能是将资源名称映射到其物理地址或属性。这种提供查询和分类服务的系统在技术术语中通常与 **目录服务器 (Directory server)** 等同使用。A、B、C 均为具体的业务服务器或中转服务器，不符合“名称映射”这一职能定义。
>
> **答案选项**：**D**
...
*   **目录服务器 (Directory Server)**：就像手机里的**通讯录**。你不需要记住每个朋友的电话号码（IP地址），只需要搜他的名字（域名/资源名），目录服务器就会告诉你怎么联系他。
...
> *   **防御**：（**之前总结过**）在交换机上开启 **动态 ARP 检测(DAI )** 。在路由器上进行 **静态 ARP 绑定**。
>
> 1.  **IP 地址冲突原因**：
