...
*   **发送限额**：发送方实际能发的字节数 = **Min(rwnd, cwnd)**。即：既要听对方的，也要看路况。
...
>![image-20251227124710397](./assets/image-20251227124710397.png)
>
>因为以前考过名词解释，总共有20个描述，20个选项，要对应起来，我认为还是有一定价值，放在第11题。
>
>**11. 读描述填关键词**
>
>| 描述                                                         | 关键词                  |
>| :----------------------------------------------------------- | :---------------------- |
>| 一种面向连接的、可靠的传输层协议                             | **TCP**                 |
>| 一种无连接的、不可靠的传输层协议                             | **UDP**                 |
>| 地址解析协议，通过 IP 地址获取其物理地址 (MAC 地址)          | **ARP**                 |
>| 用户实现互联网中电子邮件传送功能的协议                       | **SMTP**                |
>| 用于有限地理范围，将各种计算机、外设互连起来的网络           | **局域网 (LAN)**        |
>| 一个分组从源节点到达目的节点经过的路由器个数                 | **跳数 (Hop Count)**    |
>| 在一个自治系统 (AS) 内部使用的路由选择协议                   | **内部网关协议 (IGP)**  |
>| 在自治系统 (AS) 之间交换路由信息的协议                       | **外部网关协议 (BGP)**  |
>| 最初的 Internet 采用的 IGP，是一种分布式、基于距离矢量的协议 | **RIP**                 |
>| 一种路由选择算法，要求路由器维持一张包含最佳距离及输出线路的表 | **距离矢量路由算法**    |
>| 按照某种固定规则选择路由，不随当前拓扑变化的非自适应算法     | **静态路由算法**        |
>| 根据网络当前状态信息（通信量、拓扑）进行自适应调整的算法     | **动态路由算法**        |
>| 一个大的网络进行划分后形成的几个较小网络中的一部分           | **子网 (Subnet)**       |
>| 一个组织所属的几个 C 类网络合并成的一个更大的逻辑网络        | **超网 (Supernet)**     |
>| 从源主机发送报文开始到目的主机完整接收报文为止经历的时间     | **传输时延**            |
>| 每秒所传输的用户数据字节数，是在某个时间间隔内测量得到的     | **吞吐率 (Throughput)** |
>| TCP 的一种计时器，用于防止报文段丢失或被丢弃                 | **重传计时器**          |
>| 源节点发送一个分组副本到选定的一组目的节点的传输技术         | **多播 (Multicast)**    |
>| 通过连接延迟、吞吐率、残余误码率等参数描述的网络服务特征     | **服务质量 (QoS)**      |
>| 用于向 IPv4 与 IPv6 提供互操作、高质量与密码**安全**性的协议 | **IPsec**               |
>
>提示：大部分都是老生常谈，除了IPsec（其中sec表示安全Security）
