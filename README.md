...
- [南开大学 计算机研究生 计算机网络 分题型速成](#南开大学-计算机研究生-计算机网络-分题型速成)
  - [考试形式：闭卷，25选择(2分一道)+5道简答(10分一道)](#考试形式闭卷25选择2分一道5道简答10分一道)
    ...
    - [潜在真题：HTTP和DNS的简答题](#潜在真题http和dns的简答题)
    - [模拟真题：HTTP 协议深度解析](#模拟真题http-协议深度解析)
    - [模拟真题：DNS 层级结构与解析流程](#模拟真题dns-层级结构与解析流程)
    - [潜在真题：P2P和套接字编程](#潜在真题p2p和套接字编程)
    - [模拟真题：Socket 编程通信流程](#模拟真题socket-编程通信流程)
    - [潜在真题：CRC和CheckSum的描述（学长回忆补全版）](#潜在真题crc和checksum的描述学长回忆补全版)
    - [模拟真题：CRC与Checksum的计算](#模拟真题crc与checksum的计算)
    - [潜在真题：传输层两大协议对比](#潜在真题传输层两大协议对比)
    - [潜在真题：Wireshark 报文分析](#潜在真题wireshark-报文分析)
    - [模拟真题：TCP 三次握手与四次挥手](#模拟真题tcp-三次握手与四次挥手)
    - [潜在真题：IP 地址与子网划分](#潜在真题ip-地址与子网划分)
    - [模拟真题：CIDR 与子网计算](#模拟真题cidr-与子网计算)
    - [潜在真题：TCP 报文首部分析（学长回忆补全版）](#潜在真题tcp-报文首部分析学长回忆补全版)
    - [模拟真题：IPv4 报文首部分析](#模拟真题ipv4-报文首部分析)
    - [潜在真题：以太网帧与跨层协议分析（学长回忆补全版）](#潜在真题以太网帧与跨层协议分析学长回忆补全版)
    - [潜在真题：IP 地址分类与子网分析（学长回忆补全版）](#潜在真题ip-地址分类与子网分析学长回忆补全版)
    - [模拟真题：综合子网划分演练](#模拟真题综合子网划分演练)
...
> **简答题 (10 分)**
>
> 某单位分配到一个 B 类 IP 地址 `172.16.0.0`。现在需要划分为 **5 个子网**。
>
> 1.  为了满足需求，至少需要从主机位借**几位**？ (2 分)
> 2.  划分后的**子网掩码**是多少（点分十进制表示）？ (2 分)
> 3.  划分后每个子网可容纳的**最大主机数**是多少？ (2 分)
> 4.  写出**第一个子网**和**第二个子网**的网络地址。 (4 分)
>
> **简答：**
>
> 1.  **借位**：$2^2 < 5 < 2^3$，因此至少需要借 **3 位**。
> 2.  **子网掩码**：B 类默认 `/16`。借 3 位后变为 `/19`。
>     *   二进制：`11111111.11111111.11100000.00000000`。
>     *   十进制：**`255.255.224.0`**。
> 3.  **最大主机数**：主机位剩余 $32 - 19 = 13$ 位。
>     *   主机数 = $2^{13} - 2 = 8192 - 2 = $ **8190 台**。
> 4.  **子网地址**：
>     *   子网增量 = $256 / 2^3 = 32$（在第三段）。
>     *   第一个子网：**`172.16.0.0`**。
>     *   第二个子网：**`172.16.32.0`**。
