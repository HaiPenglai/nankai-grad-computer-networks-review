...
> **3.选择题**：
>
> **在数据通信中，关于同步传输模式与异步传输模式的描述，正确的是 (单选题 2分)**
> A. 异步传输以数据块为单位，需要公共时钟信号同步
> B. 异步传输在每个字符的前后分别加上起始位和停止位
> C. 同步传输的传输效率通常比异步传输低
> D. 同步传输主要用于低速、字符导向的通信场景
>
> **选项分析**：
> **异步传输**以字符为单位，通过在每个字符前后添加**起始位和停止位**来实现简单的同步，不需要复杂的全局时钟，但开销较大。**同步传输**（A、C、D 错误描述点）是以**数据块**为单位，通过公共时钟或同步字符实现，效率更高，适用于高速传输。
>
> **答案选项**：**B**
...
*   **异步传输 (Asynchronous)**：你在打联机游戏，每按下一个键，电脑就给这个键包上“头”和“尾”发送出去，键和键之间的时间间隔是不固定的。开销大。
*   **同步传输 (Synchronous)**：两台高速服务器对传大文件，它们先对准时钟，然后按照严格的节奏成块地发送数据。效率高。
