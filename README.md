...
由于南开大学 计算机研究生 计算机网络课程的PPT很难复习，而且和考试不是直接相关，而真题非常难获取。
...
**提示**：本仓库含有的PPT比较占空间，总共有100+MB，点击GitHub的Code--Download ZIP手动下载可以不下载git记录，从而省掉一些空间。

![image-20251227115909867](./assets/image-20251227115909867.png)
...
> **选项分析**：
>
> *   **客户机端**：每个客户机为了与服务器通信，至少需要打开 **1** 个套接字，N 个客户机共 **N** 个。
> *   **服务器端**：服务器需要 **1** 个**监听套接字 (Listening Socket)** 来接收新连接；此外，对于每一个已经建立连接的客户机，服务器都需要创建一个专门的**已连接套接字 (Connected Socket)** 来与其通信，即 **N** 个。
> *   **总计**：$N (客户机) + 1 (服务器监听) + N (服务器连接) = 2N + 1$。
>
> **答案选项**：**D**
